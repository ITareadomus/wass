...
// LONG (>=6h): obbligatoriamente sola, bonus più alto per compensare size=1
// SHORT (<6h): può stare con 1 task breve, bonus più basso
const STRAORDINARIA_BONUS_LONG = 700;  // >=6h (360min): porta size=1 a ~700 (quasi come size=2)
const STRAORDINARIA_BONUS_SHORT = 400; // <6h: porta size=1 a ~400, size=2 a ~900 (quasi come size=3)

export type StraordinariaInfo = {
  hasStraordinaria: boolean;
  isLong: boolean; // true se >=6h (360min)
};
