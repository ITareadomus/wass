...
def evaluate_route(route: List[Task], cleaner: "Cleaner" = None) -> Tuple[bool, List[Tuple[int, int, int]]]:
    ...
    for i, t in enumerate(route):
        ...
        if t.straordinaria:
            ...
            wait = max(0.0, t.checkout_time - effective_start)
            cur = effective_start + wait
            start = cur
...
def load_tasks() -> List[Task]:
    ...
    # Ordina: straordinarie first, poi premium, poi per DENSITÀ GEOGRAFICA (più cluster = priorità), poi checkout
    # Task con molte vicine vengono processate prima per formare cluster, task isolate processate dopo per aggregarsi
    tasks.sort(key=lambda x: (
        not x.straordinaria,
        not x.is_premium,
        -count_nearby_density(x, tasks),  # Negativo = più densità = prima (617-618-619-723 prima di 519)
        x.checkout_time
    ))
    return tasks
...
def main():
    ...
    if timeline_path.exists():
        try:
            ...
            # il filtraggio delle vecchie EO lo facciamo dentro il blocco existing_entry più sotto.
            # NON eliminare i cleaner, altrimenti perdi HP/LP/manuali!
