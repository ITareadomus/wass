...
def travel_minutes(lat1: float, lon1: float, lat2: float, lon2: float) -> float:
...
def place_after(when: datetime, t: Task) -> tuple[datetime, datetime]:
    ...
    if t.is_hp_soft:
        hp_earliest = datetime(start.year, start.month, start.day, HP_HARD_EARLIEST_H, HP_HARD_EARLIEST_M)
        start = max(start, hp_earliest)
    finish = start + timedelta(minutes=t.cleaning_time)
    return start, finish

def feasible_time_window(t: Task, start: datetime, finish: datetime) -> tuple[bool, float]:
    if t.checkin_dt is not None and finish > t.checkin_dt:
        return False, float("inf")
    penalty = 0.0
    if t.is_hp_soft and HP_SOFT_LATEST is not None:
        latest_dt = datetime(start.year, start.month, start.day, HP_SOFT_LATEST[0], HP_SOFT_LATEST[1])
        if finish > latest_dt:
            penalty += LATE_PENALTY_PER_MIN * minutes(finish - latest_dt)
    return True, penalty

def insert_cost(cleaner: Cleaner, t: Task) -> tuple[float, Optional[int], Optional[PlacedTask], float, bool]:
    ...
    if cleaner.route:
        ...
        if cleaner.last_eo_lat is not None and cleaner.last_eo_lng is not None:
            hop_min = travel_minutes(cleaner.last_eo_lat, cleaner.last_eo_lng, t.lat, t.lng)
        else:
            # fallback: if no coords, approximate using street-name similarity
            hop_min = 3.0 if same_street(cleaner.last_eo_address, t.address) else 12.0
...
def assign(tasks: List[Task], cleaners: List[Cleaner]):
    ...
    def task_key(t: Task):
        base_dt = t.checkin_dt or t.checkout_dt or datetime.now().replace(hour=HP_HARD_EARLIEST_H, minute=HP_HARD_EARLIEST_M, second=0, microsecond=0)
    ...
    for t in tasks_sorted:
        ...
        if best_feasible[1] is not None:
            ...
            unassigned.append({"task_id": t.task_id, "logistic_code": t.logistic_code, "reason": "no_eligible_cleaner_or_time_window"})
...
def seed_cleaners_from_eo(cleaners: List[Cleaner], eo_path: Path, ref_date: str):
...
def build_output(cleaners: List[Cleaner], unassigned: List[Dict[str, Any]], total_tasks: int) -> Dict[str, Any]:
    assigned_blocks = []
    cleaners_used = 0
        ...
        for idx, p in enumerate(cl.route, start=1):
            ...
            if overall_seq == 1:
                ...
                if cl.last_eo_lat is not None and cl.last_eo_lng is not None:
                    hop = travel_minutes(cl.last_eo_lat, cl.last_eo_lng, p.task.lat, p.task.lng)
                    travel_time = int(round(hop))
                else:
                    travel_time = 0 if same_street(cl.last_eo_address, p.task.address) else 12
            else:
                # hop from previous HP task
                prev = cl.route[idx-2].task
                hop = travel_minutes(prev.lat, prev.lng, p.task.lat, p.task.lng)
                travel_time = int(round(hop))
            tasks_out.append({
