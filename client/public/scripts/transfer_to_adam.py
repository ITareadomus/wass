...
# -*- coding: utf-8 -*-
"""
Script per trasferire le assegnazioni della timeline sul database ADAM (app_housekeeping)
"""
...
def transfer_to_adam(work_date: str, username: str = "system"):
    ...
    if not TIMELINE_PATH.exists():
        ...
        return {"success": False, "message": "File timeline non trovato"}

    try:
        timeline_data = json.loads(TIMELINE_PATH.read_text(encoding="utf-8"))
    except Exception as e:
        print(f"❌ Errore lettura timeline: {e}")
        return {"success": False, "message": f"Errore lettura timeline: {e}"}
    ...
    if not cleaners_assignments:
        ...
        return {"success": False, "message": "Nessuna assegnazione trovata"}
    ...
    try:
            ...
            for task in cleaner_entry.get("tasks", []):
                try:
                    ...
                    values = (update_data["checkout"],
                              ...
                              update_data["updated_at"], task_id)
                    ...
                    print(
                        f"✅ Task {logistic_code} (ID: {task_id}) trasferita su ADAM"
                    )
        ...
        return {
            "success": True,
            "message":
            f"Trasferimento completato: {total_updated} task aggiornate{f', {total_errors} errori' if total_errors > 0 else ''}",
            "stats": {
