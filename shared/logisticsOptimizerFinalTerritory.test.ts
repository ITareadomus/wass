...
import { computeTerritoryDiagnostics } from "../server/services/logistics-optimizer-final/groups/territory-diagnostics";
import {
...
import { ROUTING_SOLUTION_SCHEMA_VERSION, type RoutingSolution } from "../server/services/logistics-optimizer-final/solution-contract";
...
describe("daily territory assignment", () => {
  ...
  it("uses stronger penalties for historical templates", () => {
    expect(resolveTerritoryMismatchPenalty(0.5, HISTORICAL_TEMPLATE_PENALTY_CONFIG)).toBe(300);
    expect(resolveTerritoryMismatchPenalty(0.8, HISTORICAL_TEMPLATE_PENALTY_CONFIG)).toBe(220);
    expect(resolveTerritoryMismatchPenalty(1.1, HISTORICAL_TEMPLATE_PENALTY_CONFIG)).toBe(120);
  });

  it("makes historical territory task penalties vehicle-specific", () => {
    process.env.LOGISTICS_TERRITORY_PENALTIES = "1";
    const input = buildInput(milanLikeTasks(), buildDrivers(3, ["ADP03", "ADP01", "ADP02"]));
    const { maps } = buildOrToolsPayload(input);
    const penalties = buildVehicleTaskPenalties({
    ...
    const cseTerritory = assignment.territories.find(
      (territory) => territory.territoryKey === "center_south_east"
    )!;
    const cseTaskId = cseTerritory.taskIds[0];
    const cseTask = input.tasks.find((task) => task.taskId === cseTaskId)!;
    const northDriverId = assignment.territories.find((territory) => territory.territoryKey === "north")!
      .assignedDriverId;
    const cswDriverId = assignment.territories.find(
      (territory) => territory.territoryKey === "center_south_west"
    )!.assignedDriverId;
    const cseDriverId = cseTerritory.assignedDriverId;

    expect(penalties?.[maps.driverIdToVehicleIndex.get(cseDriverId)!][cseTask.nodeIndex]).toBe(0);
    expect(penalties?.[maps.driverIdToVehicleIndex.get(cswDriverId)!][cseTask.nodeIndex]).toBeGreaterThan(
      penalties?.[maps.driverIdToVehicleIndex.get(northDriverId)!][cseTask.nodeIndex] ?? 0
    );
  });

  it("marks non-adjacent solver territory violations as severe", () => {
    const input = buildInput(milanLikeTasks(), buildDrivers(3, ["ADP03", "ADP01", "ADP02"]));
    const assignment = input.metadata.dailyTerritoryAssignment!;
    const cseTerritory = assignment.territories.find(
      (territory) => territory.territoryKey === "center_south_east"
    ...
    const diagnostics = computeTerritoryDiagnostics(input, solution);
    expect(diagnostics?.solverTerritoryViolations[0]).toMatchObject({
      taskId: cseTaskId,
      actualDriverId: cswDriverId,
      preferredTerritoryKey: "center_south_east",
      actualDriverTerritoryKey: "center_south_west",
      adjacentTerritory: false,
      severity: "severe",
    });
    expect(diagnostics?.solverTerritoryViolations[0].actualDriverTerritoryRatio).toBeGreaterThan(0);
  });
